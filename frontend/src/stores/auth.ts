...
export const useAuthStore = defineStore('auth', () => {
  ...
  const REDIRECT_URI =
    import.meta.env.VITE_OSM_REDIRECT_URI || `${window.location.origin}/auth/callback`
  const SCOPE = 'read_prefs read_gpx write_gpx openid'
  const OSM_AUTH_URI = import.meta.env.VITE_OSM_AUTH_URI
  const OSM_TOKEN_URI = import.meta.env.VITE_OSM_TOKEN_URI
  const OSM_API_URI = import.meta.env.VITE_OSM_API_BASE_URI
  ...
  const login = async () => {
    try {
      ...
      const authUrl = `${OSM_AUTH_URI}?${params.toString()}`
  ...
  const handleCallback = async (code: string, state: string) => {
    try {
      ...
      const tokenResponse = await fetch(OSM_TOKEN_URI, {
        method: 'POST',
        headers: {
  ...
  const fetchUser = async () => {
    ...
    const response = await fetch(`${OSM_API_URI}/user/details.json`, {
      headers: {
