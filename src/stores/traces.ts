...
export const useTraceStore = defineStore('traces', () => {
	...
	const upload = async (trace: OSMTrace) => {
		try {
			...
			const { data } = await axios.post(import.meta.env.VITE_OSM_API_BASE_URI + '/gpx', {
				file: trace.file,
			...
			toast.success('Your GPX trace has been uploaded.', {
				description: 'You will recieve an email from OpenStreetMap once it\'s processed on their servers.',
				action: {
					label: 'View on OSM',
					onClick: () => window.open(`https://master.apis.dev.openstreetmap.org/user/${authStore.user?.display_name}/traces/${data}`),
				},
			})
		} catch (err) {
